...
async function loadProfile() {
  try {
    ...
    if (response.ok) {
      ...
      userDataNascimento.textContent =
        data.user.profile?.data_nascimento || "Não disponível";
      userEmail.textContent = data.user.email || "Não disponível";
      userResumo.textContent = data.user.profile?.resumo || "Não disponível";
      userLocalizacao.textContent =
        data.user.profile?.nacionalidade || "Não disponível"; // Atualizado para 'nacionalidade'
      userContato.textContent = data.user.profile?.contato || "Não disponível";

      // Exibindo os novos campos
      userEstadoCivil.textContent =
        data.user.profile?.estado_civil || "Não disponível";
      userIdentidadeGenero.textContent =
        data.user.profile?.identidade_genero || "Não disponível";
      userOrientacaoSexual.textContent =
        data.user.profile?.orientacao_sexual || "Não disponível";
      userRacaEtnia.textContent =
        data.user.profile?.raca_etnia || "Não disponível";
      userDeficiencia.textContent = data.user.profile?.deficiencia
        ? "Sim"
        : "Não"; // Para exibir 'Sim' ou 'Não'
      ...
      userPortfolio.textContent =
        data.user.profile?.portfolio || "Não disponível";
      userLinkCurriculo.textContent =
        data.user.profile?.link_curriculo || "Não disponível";
      userDescricao.textContent =
        data.user.profile?.descricao || "Não disponível";
